...
@interface FullScreenSearchViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,UISearchBarDelegate, PreviewViewDelegate, TagsViewDelegate,UITableViewDelegate, UITableViewDataSource>{
...
- (void)fetchSuggestions {
    ...
    if (text != nil && text.length > 0) {
        ...
        NSString *urlString = [NSString stringWithFormat:@"http://172.16.16.201:5001/test?suggest_key=%@", text];
//        NSString *urlString = [NSString stringWithFormat:@"http://192.168.17.119:5001/test?suggest_key=%@", text];
        ...
        NSURLSessionDataTask* task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (error == nil) {
                ...
                self->suggestionsArray = dicData[@"suggestions"];
                ...
                self->suggestionsArray = [[NSMutableArray alloc] init];
                [self.suggestionTable reloadData];
            }
        }];
        [task resume];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row < self->suggestionsArray.count) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.row < self->suggestionsArray.count) {
        NSString *key = self->suggestionsArray[indexPath.row];
        self.searchBar.text = key;
        [self searchEmojisWith:key];
    }
}
...
- (void)loadSearchGifsWithKey: (NSString *)key {
    ...
    [DongTu searchGifsWithKey:key bypassKeyWhiteList:YES At:searchPage withPageSize:pageSize completionHandler:^(NSString * _Nonnull searchKey, NSArray<DTGif *> * _Nullable gifs, DTError * _Nullable error) {
        __strong FullScreenSearchViewController *strongSelf = weakSelf;
        if (strongSelf == nil) {
            return;
        }
...
- (void)setSuggestionView {
    ...
    if (_suggestionTable == nil) {
        _suggestionTable = [UITableView new];
        _suggestionTable.delegate = self;
        _suggestionTable.dataSource = self;
        [self.view addSubview:_suggestionTable];
    }
    [_suggestionTable mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.searchContainer.mas_bottom);
        make.left.equalTo(self.view.mas_left);
        make.right.equalTo(self.view.mas_right);
        make.bottom.equalTo(self.view.mas_bottom);
    }];
    [self.view bringSubviewToFront:_suggestionTable];
}
